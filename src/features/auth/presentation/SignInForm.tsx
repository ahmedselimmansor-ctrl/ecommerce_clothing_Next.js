...
import { signInSchema, SignInInput } from '../domain/user.schema';
import { loginUserAction } from '../application/auth.service';
import { Dictionary } from '@/core/i18n/get-dictionary';

export function SignInForm({ dict, locale }: { dict: Dictionary; locale: string }) {
  ...
  const onSubmit = (data: SignInInput) => {
    startTransition(async () => {
      ...
      if (res?.error) {
        toast.error(dict.auth.error);
      } else {
        toast.success(dict.auth.successSignIn);
        router.push(`/${locale}`);
        router.refresh();
      }
    });
  ...
  return (
    <div className="w-full max-w-md mx-auto p-8 rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md shadow-2xl border border-zinc-200 dark:border-zinc-800">
      <h2 className="text-3xl font-extrabold text-zinc-900 dark:text-white mb-6 text-center">{dict.auth.signIn}</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          {isPending ? '...' : dict.auth.signIn}
        </button>

        <p className="text-center text-sm text-zinc-600 dark:text-zinc-400 mt-4">
          <a href={`/${locale}/register`} className="hover:text-indigo-500 dark:hover:text-indigo-400 font-semibold underline">
            {dict.auth.noAccount}
          </a>
        </p>
      </form>
